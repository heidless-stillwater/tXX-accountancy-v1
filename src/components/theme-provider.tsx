...
export function ThemeProvider({
  ...
  const setColorTheme = React.useCallback(
    (newCategory: ThemeCategory, newThemeName: string) => {
  ...
  React.useEffect(() => {
    ...
    const themeToClassName = (themeName: string) =>
      themeName.toLowerCase().replace(/[\s_&]+/g, '-');
  ...
  const currentThemeDef = React.useMemo(() => {
    ...
    return (
      Object.values(appThemes)
        .flat()
        .find(t => t.name === colorTheme) || null
    );
  }, [colorTheme, category]);
  ...
  const toggleMode = React.useCallback(() => {
    setMode(prevMode => {
      ...
      try {
        localStorage.setItem(
          storageKey,
          JSON.stringify({ mode: newMode, colorTheme, category })
        );
      } catch (error) {
        console.warn('Failed to save theme to localStorage:', error);
      }
